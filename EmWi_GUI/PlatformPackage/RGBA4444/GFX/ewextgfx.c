  ...
  void* aDisplay1, void* aDisplay2, void* aDisplay3 )
{
  ...
  /* determine bytes per pixel of the framebuffer depending on given color format */
    ...
    if ((int)aDisplay3 == LTDC_PIXEL_FORMAT_ARGB4444 )
      bytesPerPixel = 2;
    else  
    {
      EW_ERROR( Err07 );
  ...
  viewport->FrameBuffer->Pixel         = aDisplay1;
    ...
    Accelerator.LayerCfg[0].InputColorMode = CM_ARGB4444;
    ...
    Accelerator.LayerCfg[1].InputColorMode = CM_ARGB4444;
  ...
  int aX, int aY, int aWidth, int aHeight )
{
  ...
  srcOfs = ( srcMem.Pitch1Y / srcMem.Pitch1X ) - aWidth;
    ...
    /* transfer the modified area from canvas to the framebuffer - row by row and
       convert each pixel from native 16 bit color format into 32, 24 or 16 bit layer format */
    if ( aLtdcPixelFormat == LTDC_PIXEL_FORMAT_ARGB8888 )
    {     
      for ( ; aHeight > 0; aHeight--, src += srcOfs * 2, dst += dstOfs * 4 )
      {
        register int i;
        for ( i = 0; i < aWidth; i++, src += 2, dst += 4 )
        {
          /* copy ARGB4444 to ARGB8888 */
          register unsigned short c = *((unsigned short*)src);
          register unsigned long  d = ((unsigned long)( c & 0x000F )) | (((unsigned long)( c & 0x00F0 )) << 4 ) 
            | (((unsigned long)( c & 0x0F00 )) << 8 ) | (((unsigned long)( c & 0xF000 )) << 12 );
          *((unsigned long*)dst) = d | ( d << 4 );
        }  
      }    
	  }
    else if ( aLtdcPixelFormat == LTDC_PIXEL_FORMAT_RGB888 )
    {     
      for ( ; aHeight > 0; aHeight--, src += srcOfs * 2, dst += dstOfs * 3 )
      {
        register int i;
        for ( i = 0; i < aWidth; i++, src += 2 )
        {
          /* copy ARGB4444 to RGB888 */
          register unsigned short c = *((unsigned short*)src);
          *dst++ = (((unsigned long)( c & 0x000F )) >> 0 ) | (((unsigned long)( c & 0x000F )) << 4 );
          *dst++ = (((unsigned long)( c & 0x00F0 )) >> 4 ) | (((unsigned long)( c & 0x00F0 )) >> 0 );
          *dst++ = (((unsigned long)( c & 0x0F00 )) >> 8 ) | (((unsigned long)( c & 0x0F00 )) >> 4 );
        }  
      }    
	  }
    else if ( aLtdcPixelFormat == LTDC_PIXEL_FORMAT_RGB565 )
    {     
      for ( ; aHeight > 0; aHeight--, src += srcOfs * 2, dst += dstOfs * 2 )
      {
        register int i;
        for ( i = 0; i < aWidth; i++, src += 2, dst += 2 )
        {
          /* copy ARGB4444 to RGB565 */
          register unsigned short c = *((unsigned short*)src); 
          *((unsigned short*)dst) = (unsigned short)((( c & 0x000F ) << 1 ) | (( c & 0x0008 ) >> 3 ) 
            | (( c & 0x00F0 ) << 3 ) | (( c & 0x00C0 ) >> 1 ) | (( c & 0x0F00 ) << 4 ) | (( c & 0x0800 ) >> 4 ));
        }  
      }    
	  }
  ...
  int aWidth, int aHeight, int aBlend, unsigned long* aColors )
{
  ...
  dst    = (unsigned short*)( dstMem.Pixel1 );
  dstOfs = ( dstMem.Pitch1Y / sizeof( short )) - aWidth;
  ...
  int aBlend, unsigned long* aColors )
{
  ...
  if ( !STM32FxLockSurface( aSrcHandle, aSrcX, aSrcY, aWidth, aHeight, 0, 0, 0, 0, 0, 0, &srcMem ) )        
		return;
  src    = (unsigned short*)( srcMem.Pixel1 );
  srcOfs = ( srcMem.Pitch1Y / sizeof( short )) - aWidth;
