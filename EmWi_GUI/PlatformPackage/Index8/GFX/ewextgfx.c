...
#define Err03 "Incompatible color format of the framebuffer device! The following values " \
              ...
              "LTDC_PIXEL_FORMAT_RGB565, LTDC_PIXEL_FORMAT_ARGB4444, LTDC_PIXEL_FORMAT_L8"
...
*   separate off-screen buffer. Otherwise, only LTDC_PIXEL_FORMAT_L8 is 
*   supported.
*
* ARGUMENTS:
*   aWidth,
  ...
  void* aDisplay1, void* aDisplay2, void* aDisplay3 )
{
  ...
  /* determine bytes per pixel of the framebuffer depending on given color format */
    ...
    if ((int)aDisplay3 == LTDC_PIXEL_FORMAT_ARGB8888 )
      ...
      bytesPerPixel = 2;
    else if ((int)aDisplay3 == LTDC_PIXEL_FORMAT_L8 )
      bytesPerPixel = 1;
    else  
    {
      EW_ERROR( Err03 );
    ...
    if ((int)aDisplay3 == LTDC_PIXEL_FORMAT_L8 )
      bytesPerPixel = 1;
    else  
    {
      EW_ERROR( Err07 );
  ...
  /* determine color format of CLUT */
  if ((int)aDisplay3 == LTDC_PIXEL_FORMAT_L8 )
    ClutFormat = LTDC_PIXEL_FORMAT_ARGB8888;
  else
    ClutFormat = (int)aDisplay3;  

  /* optimize CLUT content in order to match the RGB565 or ARGB4444 framebuffer color format */
  if ( ClutFormat == LTDC_PIXEL_FORMAT_RGB565 )
  {
    register int i;
    for ( i = 0; i < 256; i++ )
    {
      register unsigned long c = Clut[ i ];
  ...
  else if ( ClutFormat == LTDC_PIXEL_FORMAT_ARGB4444 )
  {
    ...
    {
      register unsigned long c = Clut[ i ];
      Clut[ i ] = (( c & 0xF0000000 ) >> 16 ) | (( c & 0x00F00000 ) >> 12 ) | (( c & 0x0000F000 ) >> 8 ) | (( c & 0x000000F0 ) >> 4 );
    }
  }  
  ...
  int aX, int aY, int aWidth, int aHeight )
{
  ...
  {     
    for ( ; aHeight > 0; aHeight--, src += srcOfs, dst += dstOfs * 2 )
    {
      ...
      for ( i = 0; i < aWidth; i++, dst += 2 )
      {
        /* copy Index8 to ARGB4444 */
        *((unsigned short*)dst) = (unsigned short)( Clut[ *src++ ]);
      }  
    }    
  }
  else if ( aLtdcPixelFormat == LTDC_PIXEL_FORMAT_L8 )
  {     
...
*
* DESCRIPTION:
*   The function STM32FxLoadClut is called from the Graphics Engine, to load or
...
*
*******************************************************************************/
int STM32FxLoadClut( unsigned long* aColors, int aIndex, int aCount )
{
