...
*   _File  - _File stores the module name, where the object has been created.
*     _File is for debugging purpose only.
*   _Line  - _Line stores the line number, at this the object has been created.
*     _Line is for debugging purpose only.
*
*******************************************************************************/
struct _vmt_XObject;
struct _obj_XObject
...
#define EW_DEFINE_METHODS( aClass, aSuperClass )                               \
  void aClass##__Init  ( aClass _this, XObject aLink, unsigned long aArg );    \
  ...
  {                                                                            \
    ...
    const struct _XClassVariant*     _SubVariants;                             \
    int                              _Size;                                    \
    void (*_Init  )( aClass _this, XObject aLink, unsigned long aArg );        \
...
#define EW_DEFINE_VMETHODS( aClass, aSuperClass, aOriginClass )                \
  void aClass##__Init  ( aOriginClass _this, XObject aLink,                    \
                         unsigned long aArg );                                 \
  ...
  {                                                                            \
    ...
    int                               _Size;                                   \
    void (*_Init  )( aOriginClass _this, XObject aLink, unsigned long aArg );  \
...
*   aLink - Pointer to an object which aggregates (embedd) 'this' object. 
...
*******************************************************************************/
  ...
  #define EwNewObjectIndirect( aClass, aArg )                                  \
    ((XObject)EwNewObjectIndirect( aClass, aArg, __FILE__, __LINE__ ))
...
*   aLink - Pointer to an object which aggregates (embedd) aThis object. If 
...
*******************************************************************************/
  ...
  #define EwLoadResource( aResource, aClass )                                  \
    ((aClass)EwLoadResource( aResource, EW_CLASS( aClass ), __FILE__, __LINE__ ))
...
*
* RETURN VALUE:
*   Returns a string containing the given number aValue.
*
*******************************************************************************/
...
*   aIndex in the string aString. The function throws an error if the desired
*   character position aIndex is not valid.
*
...
*   aIndex in the string aString to a new value aChar. The function throws an 
*   error if the desired character position aIndex is not valid.
*
...
*
* RETURN VALUE:
*   Returns a string with the changed character.
*
*******************************************************************************/
...
*   
* ARGUMENTS:
*   aArg - An optional 32 bit argument to pass to the timer procedure.
*
* RETURN VALUE:
*   None
*
...
*   Arg          - An optional 32 bit argument to pass to the timer procedure 
*     Proc.
...
*   printed with all existing objects, their classes and the source code 
*   location where the objects have been created. 
*
*   This function works only if the macro EW_PRINT_MEMORY_USAGE is defined.
*
* ARGUMENTS:
